#!/bin/bash
#SBATCH --job-name=ckp_weather_large
#SBATCH -p segal.q
...
CHECKPOINT_FILE="v4_tsft_large_weather_checkpoint_p16_1.5pout_mse02160_mae02480"
...
python -u -m mohe_forecast.utils.run_benchmarks --model-size None \
  --block-size 672 --patch-width 16 --width-factor 1.5 --channels 21 \
  --covariates --set exp_route_dropout=0.2 \
  --epochs 30 --max-lr 2.2e-3 --min-lr 1.2e-4 \
  --weight-decay 1e-4 --warmup-portion 0.1 --setup-opt \
  --bf16 --no-moe-metrics --clip-grad 1.0 \
  --no-train --no-show-tqdm --no-save-plots --no-plot-cut-first \
  --dataset-name "Weather" --no-from-csv --batch-size 96 \
  --checkpoint-dir "$CHECKPOINT_ROOT" --checkpoint-file "$CHECKPOINT_FILE" \
  --verbose --seed 50